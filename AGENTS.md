...
- Character grid: `--cell` (1ch wide), `--line` (one line tall = `font-size × base-line-height`). A real terminal cell is taller than it is wide; this pair encodes that. **Components sit on this grid** — one-line heights are `var(--line)`, multi-line `calc(var(--line) * N)`, horizontal spacing in `ch`. Three rules keep the grid true: (1) **vertical offsets are integer `--line` multiples — no half-lines.** A `calc(var(--line) * 0.5)` seam puts content mid-row and breaks alignment; if you need spacing inside a box, step up to a full line (or use the gap between stacked boxes). (2) **Express perpendicular strokes as `box-shadow`, not `border`.** A `border: 1px` shifts the content box below it off the grid by 1px and cascades to every child. `Card`'s 2px frame, `Table`'s row dividers, and the docs sidebar/`Card`-seam borders are all `box-shadow` (inset to paint inside, outset to paint beside) so they don't take flow space. (3) **text is one cell** — `font-size: var(--font-size)`, `line-height: var(--line)`; hierarchy comes from uppercase/weight/spacing, not size. A larger `font-size` with a different line-height puts that element's line-box off the grid. There is a **GRID** toggle in the docs sidebar (mostly the `_theme-picker`) that paints the `--cell`×`--line` grid over the page — flip it on while building to verify a component still snaps. Use it before claiming a component is grid-aligned.
